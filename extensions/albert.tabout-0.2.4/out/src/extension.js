...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('toggle-tabout-reverse-shift-tab', () => __awaiter(this, void 0, void 0, function* () {
        let taboutConfiguration = vscode.workspace.getConfiguration('tabout');
        let currentReverseEnabled = taboutConfiguration.get('enableReverseShiftTab', true);
        yield taboutConfiguration.update('enableReverseShiftTab', !currentReverseEnabled, true);
        vscode_1.window.showInformationMessage("TabOut reverse Shift+Tab is " + (!currentReverseEnabled ? "" : " NOT ") + "active");
    })));
    context.subscriptions.push(vscode.commands.registerCommand('tabout-reverse', () => {
        ...
        if (currentPositionInLine == 0) {
            return vscode_1.commands.executeCommand("outdent");
        }
        return utils_1.selectPreviousCharacter(currentLineText, currentPositionInLine);
    }));
