...
function updateCSS() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!configuration.get('markdownStyle')) {
            yield Promise.all(files.map((file) => vscode_1.workspace.fs.writeFile(vscode_1.Uri.file(getCSSPath(file)), new util_1.TextEncoder().encode(''))));
        }
        else {
            yield Promise.all(files.map((file) => __awaiter(this, void 0, void 0, function* () {
                const fileContents = yield vscode_1.workspace.fs.readFile(vscode_1.Uri.file(getCSSPath(`./origin/${file}`)));
                return vscode_1.workspace.fs.writeFile(vscode_1.Uri.file(getCSSPath(`./${file}`)), fileContents);
            })));
        }
    });
}
