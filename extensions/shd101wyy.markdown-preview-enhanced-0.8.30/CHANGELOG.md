...
- **`Follow link` (alt+click / Ctrl+click) on `[[wikilinks]]`** — VS Code's built-in markdown link provider already handled `[text](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/blob/HEAD/Note.md)`; the new `WikilinkDocumentLinkProvider` extends the same UX to `[[Note]]`, `![[Note]]`, `[[Note#Heading]]`, and `[[Note^block]]`. Click navigates to the target file (auto-creating it if missing — see below) and jumps to the heading/block line for fragment-bearing links.
